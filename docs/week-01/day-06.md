...
✅ Declare functions with `fn name(param: Type) -> ReturnType { ... }`: parameter types are always mandatory, and a return type is required whenever the function returns a value  
...
::: warning Watch Out!
- **The stray semicolon.** Writing `x * 2;` as the last line of a function that declares `-> i32` fails with "mismatched types: expected `i32`, found `()`". The semicolon converted your return expression into a statement. This is the single most common beginner error with functions, and it does NOT compile.
