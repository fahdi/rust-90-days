# Week 5: Structs & Enums I

Build complex data structures and handle different states.

## Learning Objectives

By the end of this week, you'll have mastered key concepts in structs & enums i.

## Daily Breakdown

| Day | Topic | Time | Difficulty |
|-----|-------|------|------------|
| [Day 29](/week-05/day-29) | Defining Structs | 10 min | ⭐⭐ |
| [Day 30](/week-05/day-30) | Struct Methods | 10 min | ⭐⭐ |
| [Day 31](/week-05/day-31) | Associated Functions | 9 min | ⭐⭐ |
| [Day 32](/week-05/day-32) | Tuple Structs | 9 min | ⭐⭐ |
| [Day 33](/week-05/day-33) | Enums Basics | 10 min | ⭐⭐ |
| [Day 34](/week-05/day-34) | Option&lt;T&gt; | 10 min | ⭐⭐ |
| [Day 35](/week-05/day-35) | Result&lt;T, E&gt; | 10 min | ⭐⭐ |

## What Makes This Week Important

These concepts allow you to build robust, type-safe applications.

## Study Tips for This Week

1. **Practice daily** - Consistency is key to mastering these concepts
2. **Experiment freely** - Try variations of the examples
3. **Read error messages** - The compiler is your best teacher
4. **Build something** - Apply concepts in a small project

---

[Start Day 29: Defining Structs →](/week-05/day-29)
